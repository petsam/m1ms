...
# Help message
SfsHelp="M1mS creates Manjaro mirrorlist and changes mirrors safely
    Options:
...
SfsMirrorlistHeading="##### Generated by M1mS #####
"
...
if [ $# -eq 0 ]; then
	...
	DateSuffix=$(date +%s)
	if [ -w /tmp ];then
		WorkDir=/tmp/m1ms-"$DateSuffix"
	else
		WorkDir=$HOME/.local/tmp/m1ms-"$DateSuffix"
	fi
	mkdir -p "$WorkDir"
	if [ -d "$WorkDir" ]; then
	...
	if [ $EnabledServers -eq 0 ]; then
		echo "There is no enabled Server in the mirrorlist. Recreate mirrorlist with \"m1ms -i\" "
	fi
	exit;
    ...
    # Create work directory
	if [ -w /tmp ];then
		WorkDir=/tmp/m1ms-$(date +%s)
	else
		WorkDir=$HOME/.local/tmp/m1ms-$(date +%s)
	fi
	mkdir -p "$WorkDir"
	if [ -d "$WorkDir" ]; then
		echo "Created working directory :" "$WorkDir"
    ...
    if [ -s result.log ]; then
	  ...
	  while true; do
		...
		[Yy]) echo $SfsMirrorlistHeading > mirrorlist
			  ...
			  # TODO run m1ms -n to check validity of first mirror
